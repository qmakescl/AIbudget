...
### 6.1 `정부안` 열 잘못된 매핑 버그 (2026-03-10 발견 및 수정)

#### 발견 경위

고용노동부 19번(노동위원회정보화운영) ~ 26번(취약노동자지원) 사업을 원문 PDF와 대조 검토하던 중, `verify_extract.py`가 `budget_2026_req`를 읽지 못하고 `budget_2026`에 잘못된 값을 저장하는 오류 발견.

#### 원인

**8열 총괄표 구조** — 일부 부처(고용노동부 등)는 2026년 예산 열이 "정부안"과 "확정(B)" 두 열로 분리된 8열 구조를 사용:

```text
사업명 | 2024결산 | 2025본예산(A) | 2025추경 | 2026정부안 | 2026확정(B) | 증감(B-A) | 증감률
```

`detect_column_structure`의 `budget_2026` 탐지 패턴에 "정부안"이 포함되어 있어, "정부안" 열(요구안)이 `budget_2026`으로 먼저 매핑되고 실제 "확정(B)" 열이 무시됐다:

```python
# 수정 전 — "정부안"이 budget_2026 패턴에 포함됨
...
`extraction_audit.csv`의 MISMATCH 항목 전수 분석 결과 `column_count=8`인 25건 모두 동일 버그:

| 부처 | 영향 사업수 |
| --- | --- |
| 고용노동부 | 4건 (19, 23, 24, 26번) |
| 기후에너지환경부 | 8건 |
| 국토교통부 | 3건 |
| 경찰청, 과학기술정보통신부, 교육부, 보건복지부, 질병관리청, 해양수산부, 기획예산처 | 각 1~3건 |

PyMuPDF는 텍스트를 순서대로 읽어 7열 구조로 정확히 매핑했으나, 교차 검증 시 pdfplumber 결과가 우선 적용되어 오류값이 최종 저장됐다.

#### 수정 내용

`verify_extract.py` `detect_column_structure` 함수 (122~125번째 줄):

```python
# 수정 후 — "정부안"을 budget_2026_req 패턴으로 이동
...
- "정부안" 열 → `budget_2026_req` 로 정확히 분류
- "확정(B)" 열 → `budget_2026` 로 정확히 분류
- "정부안"만 있는 6열 표 → `budget_2026_req` + `2026` 폴백으로 `budget_2026`도 동일 열에 매핑 (기존 동작 유지)
...
| --- | --- | --- |
| MISMATCH | 25건 | 2건 |
| 고용노동부 19번 budget_2026 | 5,626 (정부안, 오류) | 2,959 (확정, 정확) |
| 고용노동부 23번 budget_2026 | 9,601 (정부안, 오류) | 11,101 (확정, 정확) |
| 고용노동부 24번 budget_2026 | 78,970 (정부안, 오류) | 82,080 (확정, 정확) |
| 고용노동부 26번 budget_2026 | 14,795 (정부안, 오류) | 25,345 (확정, 정확) |

잔여 MISMATCH 2건(경찰청 5번, 교육부 220번)은 `validation_flags=OK`로 산술 검증 통과 — 열 매핑 버그가 아닌 pdfplumber/PyMuPDF 간 미세 수치 차이.

### 6.2 동결사업 증감/증감률 결측 보정 (2026-03-10 발견 및 수정)

#### 6.2 발견 경위

과학기술정보통신부 45번(AI기반 맞춤형 케어서비스 융합선도) 등 원문 PDF를 직접 확인하던 중, 2025년과 2026년 예산이 동일한 동결사업에서 PDF 원본이 증감(B-A)과 (B-A)/A 열을 숫자 `0`이 아닌 `-`로 표기하는 사례 발견.

예시 (PDF 원본):

```text
사업명                          | 2024결산 | 2025본예산 | 2025추경(A) | 2026요구안 | 2026본예산(B) | 증감(B-A) | (B-A)/A
AI기반 맞춤형 케어서비스 융합선도(R&D) |    -    |   3,000   |   3,000    |   3,000   |    3,000    |    -     |    -
```

#### 6.2 원인

`parse_number`는 `-` 단독을 `None`으로 반환하도록 설계되어 있어, 원본에서 `-`로 표기된 증감/증감률이 `NaN`으로 저장됐다.

#### 6.2 영향 범위

기존사업(`is_new=False`) 중 `budget_2026` 있고 `change_amount=NaN`인 케이스 **16건** (과학기술정보통신부 8건, 해양수산부 4건, 보건복지부 2건, 경찰청·교육부·중소벤처기업부·방송미디어통신위원회 각 1건).

#### 6.2 수정 내용

`verify_extract.py` `extract_project` 함수 내 신규사업 보정 로직 이후 추가:
...
| 항목 | 수정 전 | 수정 후 |
| --- | --- | --- |
| 기존사업 중 change_amount 결측 | 16건 | 0건 |
| 증감액=0 동결 사업 | - | 19건 |

- 보정 적용 범위: `is_new=False` & `budget_2026`/`actual_2025` 모두 존재 & `change_amount=None`
- 신규사업(`is_new=True`)에는 적용하지 않아 기존 신규사업 보정 로직과 충돌 없음
